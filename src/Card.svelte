...
@media (hover: hover) {
    article:not([disabled=true]):not(.show):hover {
        transform: perspective(400px) rotateY(18deg) translateZ(1vw);
    ...
    article.selected {
            transform: perspective(400px) rotateY(180deg) scale(1.4) translateZ(-5vw);
            z-index:100;
    }
...
/* Faces */
    section {
        ...
        pointer-events: none;
        /* border: clamp(4px, calc(5vw / var(--col-count)), 10px) solid #666; */
        transition:all 0.3s;
        border-radius:calc(10vw / var(--col-count));
        cursor: pointer;
        display: grid;
    ...
    section.back {
        filter: saturate(0);
        background: repeating-linear-gradient(45deg, #e66465, #e66466 1vw, white 1vw, white 2vw);

                    /* background: repeating-linear-gradient(45deg, #e66465, #e66466 10px, white 10px, white 2 0px);
    */
